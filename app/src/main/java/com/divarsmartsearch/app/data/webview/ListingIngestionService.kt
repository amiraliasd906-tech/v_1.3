    ...
    suspend fun ingest(savedSearchId: Long, items: List<ExtractedListing>): IngestResult {
        ...
        for (item in items) {
            ...
            if (existing == null) {
                ...
                // every cycle, which reset isVisible = true and sent it
                // back through the ENTIRE filter pipeline -- including a
                // fresh agency/owner classification that could come out
                // differently by chance and hide a listing that never
                // actually changed. Once a listing has a real detail
